...
if(isset($_POST['approve'])){
    ...
    $edit = db_update('tbl_orders', $data, $id = array("id"=>$_GET['id']));
    if(isset($edit)){
        db_insert('tbl_logs', $data = array("transaction"=>$_POST['or_no'].' Order has been Marked as Approved', "created_by"=>$_SESSION['username']));
        $to = $_POST['contact'];
        $smsBody = "Your Order ". strtoupper($_POST['or_no']) . " is now on Process. Your 50% Down payment was acknowledged by our team.";
        $client->messages->create($to,array('from' => $from,  'body' => $smsBody));
        $_SESSION['toastr'] = array(
            'type'=>'info',
            'message'=>'Order has been accepted',
...
if(isset($_POST['delivery'])){
    ...
    $edit = db_update('tbl_orders', $data, $id = array("id"=>$_GET['id']));
    if(isset($edit)){
        db_insert('tbl_logs', $data = array("transaction"=>$_POST['or_no'].' Order has been Marked for Delivery', "created_by"=>$_SESSION['username']));

        $to = $_POST['contact'];
        $smsBody = "Your Order ". strtoupper($_POST['or_no']) . " is for delivery today. Please expect our delivery team & be ready";
        $client->messages->create($to,array('from' => $from,  'body' => $smsBody));
        $_SESSION['toastr'] = array(
            'type'=>'info',
...
if(isset($_POST['claimed'])){
    ...
    $edit = db_update('tbl_orders', $data, $id = array("id"=>$_GET['id']));
    if(isset($edit)){
        db_insert('tbl_logs', $data = array("transaction"=>$_POST['or_no'].' Order has been Marked as Claimed', "created_by"=>$_SESSION['username']));
...
if(isset($_POST['paid'])){
    ...
    $edit = db_update('tbl_orders', $data, $id = array("id"=>$_GET['id']));
    if(isset($edit)){
        db_insert('tbl_logs', $data = array("transaction"=>$_POST['or_no'].' Order has been Marked as Paid', "created_by"=>$_SESSION['username']));

        $to = $_POST['contact'];
        $smsBody = "Your Order ". strtoupper($_POST['or_no']) . " has been Fully Paid. Thank you for purchasing our item, please wait your item to be deliver";
        $client->messages->create($to,array('from' => $from,  'body' => $smsBody));
        $_SESSION['toastr'] = array(
            'type'=>'info',
...
<!--                                        <th>Order Type</th>-->
                                    ...
                                    <?php
                                    $xid = $_SESSION['id'];
                                    $value = custom_query("SELECT * FROM tbl_orders WHERE status !='Claimed' ORDER BY id DESC");
                                    if($value->rowCount()>0)
                                    {
...
<!--                                                <td>--><?//= $r['prod_category']; ?><!--</td>-->
                                                ...
                                                <td><?= $r['prod_color']; ?></td>
<!--                                                <td>--><?//= $r['or_type']; ?><!--</td>-->
                                                <td><?php if($r['status'] =='Delivery'){echo "Delivering";} else { echo $r['status']; }; ?></td>
                                                <td>
...
<!--                                                        <button type="button" class="btn btn-success mr-3" title="Edit" data-target="#accept-record--><?//=$id; ?><!--" data-toggle="modal"><i class="fa fa-check"></i></button>-->
                                                        <button type="button" data-type="confirm" class="btn btn-danger js-sweetalert" data-target="#paid-record<?=$id; ?>" data-toggle="modal"><i class="fa fa-money"></i></button>
                                                        <button type="button" class="btn btn-info mr-3" data-target="#delivery-record<?=$id; ?>" data-toggle="modal"><i class="fa fa-bug"></i></button>
...
<!--                                                        <button type="button" class="btn btn-warning mr-3" title="View Proof" data-target="#terms-record--><?//=$id; ?><!--" data-toggle="modal"><i class="fa fa-eye"></i></button>-->
                                                    ...
                                                    <?php if($r['status'] == 'Delivery'): ?>
                                                        <button type="button" class="btn btn-info mr-3" data-target="#claim-record<?=$id; ?>" data-toggle="modal"><i class="fa fa-bug"></i></button>
                                                    <?php endif; ?>
                                                    <?php if($r['status'] == 'Pending'): ?>
                                                        <button type="button" class="btn btn-success mr-3" data-target="#approve-record<?=$id; ?>" data-toggle="modal"><i class="fa fa-check"></i></button>
                                                        <?php if($r['reservation_proof'] != ''): ?>
                                                        <button type="button" class="btn btn-warning mr-3" data-target="#terms-record<?=$id; ?>" data-toggle="modal"><i class="fa fa-eye"></i></button>
                                                        <?php endif; ?>
                                                        <?php if($r['reservation_proof'] == ''): ?>
                                                        <button type="button" class="btn btn-info" data-target="#downpayment-record<?=$id; ?>" data-toggle="modal"><i class="fa fa-money"></i></button>
                                                            <?php endif; ?>
                ...
                <div class="modal-content">
                    <form action="?id=<?= $id; ?>" method="POST" enctype="multipart/form-data">
                        <div class="modal-header">
                            <h4 class="title" id="defaultModalLabel">Upload Down Payment?</h4>
                        </div>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form action="?id=<?= $id; ?>" method="POST">
                        <div class="modal-header">
                            <h4 class="title" id="defaultModalLabel">Client Proof of Payment</h4>
                        </div>
                        <div class="modal-body">
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form action="?id=<?= $id; ?>" method="POST">
                        ...
                        <div class="modal-body">
                            <input type="hidden" value="<?= $r['contact']; ?>" name="contact" />
                            <input type="hidden" value="<?= $r['or_no']; ?>" name="or_no" />
