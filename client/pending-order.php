...
//    move_uploaded_file($filetmp,$terms);
    ...
    if (isset($edit)) {
        ...
        $_SESSION['toastr'] = array(
            'type' => 'warning',
            'message' => 'Downpayment is Sent Successfully',
            'title' => 'Congratulations'
        );
        redirect('pending-order.php');
        exit();
...
<!--                                        <th>Order Type</th>-->
                                    ...
                                    <?php
                                    $xid = $_SESSION['id'];
                                    $value = custom_query("SELECT * FROM tbl_orders WHERE user_id = '$xid' ORDER BY id DESC LIMIT 5");
                                    if($value->rowCount()>0)
                                    {
    ...
    while($r=$value->fetch(PDO::FETCH_ASSOC))
    {
        ...
        <div class="modal fade" id="rate-record<?= $id; ?>" tabindex="-1" role="dialog">
            <div class="modal-dialog modal-sm" role="document">
                <div class="modal-content">
                    <form action="?id=<?= $r['product_id']; ?>&name=<?= $name; ?>" method="POST">
                        <div class="modal-header">
