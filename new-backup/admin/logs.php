...
if(isset($_POST['create'])){
    ...
    if(isExists('tbl_types', $data = array("title"=>$_POST['title'], "description"=>$_POST['description'])) == 1){
        $error[] = "Type title or same description already exists";
    }
    else{
        $insert = db_insert('tbl_types', $data);
        if(isset($insert)){
...
if(isset($_POST['edit'])){
    ...
    $edit = db_update('tbl_types', $data, $id = array("id"=>$_GET['id']));
    if(isset($edit)){
...
if(isset($_POST['delete'])){
    $delete = db_delete('tbl_types', $id = array("id"=>$_GET['id']));
    if(isset($delete)){
            ...
            <div class="block-header">
                <div class="row">
                    <div class="col-lg-5 col-md-8 col-sm-12">
                        <h2><a href="javascript:void(0);" class="btn btn-xs btn-link btn-toggle-fullwidth"><i class="fa fa-arrow-left"></i></a> Logs Management</h2>
                        <ul class="breadcrumb">
                        ...
                        <div class="body">
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover js-basic-example dataTable table-custom">
                                    ...
                                    <?php
                                    $xid = $_SESSION['id'];
                                    $value = custom_query("SELECT * FROM tbl_logs ORDER BY id DESC");
                                    if($value->rowCount()>0)
                                    {
                                        while($r=$value->fetch(PDO::FETCH_ASSOC))
                                        {
                                            ...
                                            <tr>
                                                <td><?= $r['transaction']; ?></td>
                                                <td><?php custom_echo($r['created_by'], 25); ?></td>
                                                <td><?= $r['dtcreated']; ?></td>
                                            </tr>
                                        <?php }} ?>
                                    </tbody>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form action="?id=<?= $id; ?>" method="POST">
                        ...
                        <div class="modal-body"> Are you sure you want to remove this Type?</div>
                        <div class="modal-footer">
                ...
                <div class="modal-content">
                    <form action="?id=<?= $id; ?>" method="POST">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label>Description</label>
                                <textarea name="description" class="form-control" required rows="5"><?= $r['description']; ?></textarea>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-danger" data-dismiss="modal">CANCEL</button>
